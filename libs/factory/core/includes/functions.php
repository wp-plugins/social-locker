 ...
 * @return string
 */
function factory_309_get_nonce() {
 ...
 * @return string
 */
function factory_309_nonce() {
    echo factory_309_get_nonce();
}
 ...
 * @return bool
 */
function factory_309_starts_with($haystack, $needle) {
 ...
 * @return bool
 */
function factory_309_ends_with($haystack, $needle) {
...
function factory_309_json_error($error) {
    echo json_encode(array('error' => $error));
    exit;
}

function factory_309_json_success( $data = array() ) {
    $data['error'] = false;
 ...
 * @return string[]
 */
function factory_309_pathinfo( $path ) {
    $data = pathinfo($path);
    $data['filename'] = factory_309_filename_without_ext($data['basename']);
    return $data;
}
 ...
 * @return string
 */
function factory_309_filename_without_ext($filename){
 ...
 * @return string
 */
function factory_309_filename_ext($filename){
    $result = explode('.', $filename);
    if ( empty($result) ) return null;
    return $result[count($result)-1];
}

function factory_309_get_human_filesize_by_path( $path ) {
    $bytes = filesize( $path );
    return factory_get_human_filesize( $bytes );
}

function factory_309_get_human_filesize( $bytes ) {
    if ($bytes >= 1073741824)
    {
 ...
 * @since 1.0.0
 */
function factory_309_get_days( $startDate, $endDate ) {
 ...
 * @since 1.0.0
 */
function factory_309_get_weeks( $startDate, $endDate ) {
    $days = factory_309_get_days( $startDate, $endDate );
 ...
 * @since 1.0.0
 */
function factory_309_get_months( $startDate, $endDate ) {
 ...
 * @return type
 */
function factory_309_is_administrator() {
 ...
 * @return void
 */
function factory_309_print_error( $title, $message ) {
    ?>
 ...
 * @return void
 */
function factory_309_iframe_resize_script( $wrapId ) {
    ?>
    <script>
 ...
 * @return void
 */
function factory_309_set_lazy_redirect( $url ) {
    update_option('factory_lazy_redirect', $url );
}

add_action('admin_init', 'factory_309_do_lazy_redirect');
function factory_309_do_lazy_redirect() {
