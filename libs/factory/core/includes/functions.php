 ...
 * @return string
 */
function factory_311_get_nonce() {
 ...
 * @return string
 */
function factory_311_nonce() {
    echo factory_311_get_nonce();
}
 ...
 * @return bool
 */
function factory_311_starts_with($haystack, $needle) {
 ...
 * @return bool
 */
function factory_311_ends_with($haystack, $needle) {
...
function factory_311_json_error($error) {
    echo json_encode(array('error' => $error));
    exit;
}

function factory_311_json_success( $data = array() ) {
    $data['error'] = false;
 ...
 * @return string[]
 */
function factory_311_pathinfo( $path ) {
    $data = pathinfo($path);
    $data['filename'] = factory_311_filename_without_ext($data['basename']);
    return $data;
}
 ...
 * @return string
 */
function factory_311_filename_without_ext($filename){
 ...
 * @return string
 */
function factory_311_filename_ext($filename){
    $result = explode('.', $filename);
    if ( empty($result) ) return null;
    return $result[count($result)-1];
}

function factory_311_get_human_filesize_by_path( $path ) {
    $bytes = filesize( $path );
    return factory_get_human_filesize( $bytes );
}

function factory_311_get_human_filesize( $bytes ) {
    if ($bytes >= 1073741824)
    {
 ...
 * @since 1.0.0
 */
function factory_311_get_days( $startDate, $endDate ) {
 ...
 * @since 1.0.0
 */
function factory_311_get_weeks( $startDate, $endDate ) {
    $days = factory_311_get_days( $startDate, $endDate );
 ...
 * @since 1.0.0
 */
function factory_311_get_months( $startDate, $endDate ) {
 ...
 * @return type
 */
function factory_311_is_administrator() {
 ...
 * @return void
 */
function factory_311_print_error( $title, $message ) {
    ?>
 ...
 * @return void
 */
function factory_311_iframe_resize_script( $wrapId ) {
    ?>
    <script>
 ...
 * @return void
 */
function factory_311_set_lazy_redirect( $url ) {
    update_option('factory_lazy_redirect', $url );
}

add_action('admin_init', 'factory_311_do_lazy_redirect');
function factory_311_do_lazy_redirect() {
