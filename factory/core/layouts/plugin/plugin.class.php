...
class FactoryPR108Plugin {
    ...
    public function actionPluginLoadded() {
        ...
        do_action('factory_pr108_init', $this);
    }
    ...
    public function actionInit() {
        ...
        $this->shortcodes = new FactoryPR108ShortcodeManager( $this );   
        $this->metaboxes = new FactoryPR108MetaboxManager( $this );   
        
        if ( $this->isAdmin ) {
            
            $this->notices = new FactoryPR108NoticeManager( $this );
            $this->pages = new FactoryPR108AdminPageManager( $this ); 
    ...
    public function activationOrUpdateHook( $forceActivation = false ) {
        do_action('factory_pr108_activation_or_update-' . $this->pluginName);
    ...
    public function deactivationHook() {
        do_action('factory_pr108_deactivation-' . $this->pluginName);;
	...
	global $post;
        ...
        wp_enqueue_style('factory-admin-global', FACTORY_PR108_URL . '/assets/css/admin-global.css');
        wp_enqueue_script('factory-admin-global', FACTORY_PR108_URL . '/assets/js/admin-global.js'); 
                        
	if ( in_array( $hook, array('post.php', 'post-new.php')) && $post )
        {
            if ( !empty( $this->types[$post->post_type] ) ) {
                
		wp_enqueue_style('factory-bootstrap', FACTORY_PR108_URL . '/assets/css/bootstrap.css');	
		wp_enqueue_script('factory-bootstrap', FACTORY_PR108_URL . '/assets/js/bootstrap.js', array('jquery'));
            }
    ...
    public function load( $path, $name ) {
        ...
        do_action('factory_pr108_load_' . $name, $this);
    }
    ...
    public function showCustomPluginRow($file, $plugin_data) {
        ...
        $messages = apply_filters('factory_pr108_plugin_row-' . $this->pluginName, array(), $file, $plugin_data);
